  ...
  const PlaySection = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div className="glass-card" style={{ padding: '24px' }}>
        ...
        {error && (
          <div className="slide-up" style={{ marginTop: '14px', padding: '14px 16px', background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: '12px', color: '#f87171', fontSize: '14px' }}>
            {error}
          </div>
        )}

        {result && resultMsg && (
          <div className="slide-up" style={{ marginTop: '14px', padding: '18px', background: result.isUnique ? 'rgba(34,197,94,0.08)' : 'rgba(251,191,36,0.08)', border: `1px solid ${result.isUnique ? 'rgba(34,197,94,0.25)' : 'rgba(251,191,36,0.25)'}`, borderRadius: '14px' }}>
        ...
        {attemptsRemaining === 0 && (
          <div style={{ marginTop: '14px', padding: '18px', textAlign: 'center', background: 'rgba(127,119,221,0.08)', border: '1px solid rgba(127,119,221,0.2)', borderRadius: '14px' }}>
            ...
            <button onClick={() => router.replace('/premium')} className="btn-owl" style={{ padding: '10px 20px', fontSize: '13px' }}>
              ⚡ Get more attempts
            </button>
          </div>
        )}
      ...
      <div className="glass-card" style={{ padding: '20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <div onClick={() => router.replace('/room?type=guessme&action=create')}
            style={{ background: 'linear-gradient(135deg, rgba(45,27,105,0.6), rgba(26,15,60,0.6))', border: '1px solid rgba(127,119,221,0.2)', borderRadius: '14px', padding: '16px', cursor: 'pointer', transition: 'all 0.2s' }}
            onMouseEnter={e => e.currentTarget.style.border='1px solid rgba(127,119,221,0.5)'}
            onMouseLeave={e => e.currentTarget.style.border='1px solid rgba(127,119,221,0.2)'}>
          ...
          <div onClick={() => router.replace('/room?type=bigbrains&action=create')}
            ...
            onMouseLeave={e => e.currentTarget.style.border='1px solid rgba(245,158,11,0.2)'}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
    ...
    <div className="glass-card" style={{ padding: '20px', height: 'fit-content', position: 'sticky', top: '80px' }}>
      ...
      {leaderboard.length > 0 ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {leaderboard.map((p, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 14px', background: i === 0 ? 'rgba(255,215,0,0.08)' : 'rgba(255,255,255,0.03)', border: i === 0 ? '1px solid rgba(255,215,0,0.2)' : '1px solid rgba(255,255,255,0.05)', borderRadius: '14px', transition: 'all 0.2s' }}
              onMouseEnter={e => e.currentTarget.style.background = 'rgba(127,119,221,0.1)'}
              onMouseLeave={e => e.currentTarget.style.background = i === 0 ? 'rgba(255,215,0,0.08)' : 'rgba(255,255,255,0.03)'}>
